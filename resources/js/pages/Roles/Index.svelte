<script lang="ts">
    ...
    function permissionLabel(resource: string): string {
    ...
    function actionLabel(action: string): string {
        ...
        return labels[action] ?? action.charAt(0).toUpperCase();
    }
</script>
                            ...
                            <td class="px-6 py-4">
                                {#if Object.keys(role.permissions).length > 0}
                                    <div class="flex flex-wrap gap-1.5">
                                        {#each Object.entries(role.permissions) as [resource, actions] (resource)}
                                            <span
                                                class="inline-flex items-center gap-1 rounded-md bg-gray-50 px-2 py-0.5 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700"
                                            >
                                                {permissionLabel(resource)}:
                                                {#each actions as string[] as action, i (action)}
                                                    <span
                                                        class="font-mono text-xs {action ===
                                                        'delete'
                                                            ? 'text-red-500'
                                                            : action ===
                                                                'create'
                                                    ...
                                                    >
                                                        {actionLabel(
                                                            action,
                                                        )}{#if i < (actions as string[]).length - 1},{/if}
                                                    </span>
                                                {/each}
                                            </span>
                                        {/each}
